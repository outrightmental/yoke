...
In Simple SDLC mode — the default when `GITHUB_PROJECT_NUMBER` is not set — Yoke runs fully autonomously from issue to merged PR:
...
Set `YOKE_REVIEWERS` to a comma-separated list of GitHub logins to notify when a PR is ready.
...
```bash
export YOKE_GITHUB_TOKEN=github_pat_...
claude login
```
...
- Node.js 18+
- `git` on `PATH`.
- A GitHub PAT exposed as `YOKE_GITHUB_TOKEN` or `GITHUB_TOKEN`.
...
Yoke uses a GitHub PAT directly for API calls and Git clone/fetch/push operations. Claude Code authentication is still handled by the `claude` CLI.
...
| --- | --- | --- |
| `YOKE_GITHUB_TOKEN` | — | Preferred GitHub PAT used by Yoke for API and Git operations. |
| `GITHUB_TOKEN` | — | Fallback GitHub token / PAT used when `YOKE_GITHUB_TOKEN` is unset. |
...
| `YOKE_SESSION_STORE_PATH` | `<cwd>/.yoke/<owner>-<repo>-sessions.json` | Path for persisted local agent-session state. |
| `GITHUB_PROJECT_NUMBER` | — | GitHub Projects v2 board number. Enables [Project SDLC](#project-sdlc-human-in-the-loop). |
| `YOKE_REVIEWERS` | — | Comma-separated GitHub logins to request review from (Project SDLC only). |
